...
import utils
# 이부분 바뀜
from data_utils_v5_4 import (
  TextAudioSpeakerLoader,
  TextAudioSpeakerCollate,
  DistributedBucketSampler
)
from models_v5_4 import (
  SynthesizerTrn,
...
def main():
  ...
  parser.add_argument('-c', '--config', type=str, default="/home/sim/VoiceConversion/V5_2/freevc_v5_3.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str, default="V5_4",
                      help='Model name')
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q), (emb, emb_quantized, perplexity), g = net_g(c, spec, g=g, mel=mel_H)
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_commitment, loss_codebook = vq_loss(emb, emb_quantized, posterior_emb=z_p) #VQ_loss
        # loss_contrastive = info_nce_loss(vq_spk_emb, spk_emb) #Contrastive loss
        loss_contrastive = torch.tensor([0]).cuda(rank)

        loss_gen_all = loss_gen + loss_fm + loss_mel + loss_kl + loss_commitment + loss_codebook+ loss_contrastive
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_kl, loss_commitment, loss_codebook, loss_contrastive, perplexity]
...
def evaluate(hps, nets, eval_loader, writer_eval):
    ...
    with torch.no_grad():
      ...
      y_hat = generator.module.infer(c, g=g, mel=mel_H)
