...
class Encoder(nn.Module):
  def __init__(self,
    ...
    if vq_codebook_size != None:
        self.codebook = modules.VQEmbeddingEMA(hidden_channels, vq_codebook_size)
  ...
  def forward(self, x, x_lengths, g=None):
    ...
    if self.vq_codebook_size==None: 
        ...
        m, m_quan, perplexity = self.codebook(m)
        z = (m_quan + torch.randn_like(m) * torch.exp(logs)) * x_mask # Gaussian Distribution 
    
    if self.vq_codebook_size == None:
        return z, m, logs, x_mask
    else:
        return z, m, logs, x_mask, (m, m_quan), perplexity
...
class SynthesizerTrn(nn.Module):
  ...
  def forward(self, c, spec, g=None, mel=None, c_lengths=None, spec_lengths=None):
    ...
    _, m_p, logs_p, _, (emb, emb_quantized), perplexity = self.enc_p(c, c_lengths) # _p: prior  distribution
    ...
    # z_p_slice = commons.slice_segments(z_p, ids_slice, self.segment_size)
    # emb_slice = commons.slice_segments(emb_slice, ids_slice, self.segment_size)
    # emb_quan_slice = commons.slice_segments(emb_quantized, ids_slice, self.segment_size)
