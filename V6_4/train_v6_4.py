...
import utils
# 이부분 바뀜
from data_utils_v6_4 import (
  TextAudioSpeakerLoader,
  TextAudioSpeakerCollate,
  DistributedBucketSampler
)
from models_v6_4 import (
  SynthesizerTrn,
...
def main():
  ...
  parser.add_argument('-c', '--config', type=str, default="/home/sim/VoiceConversion/V6_4/freevc_v6_4.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str, default="V6_4",
                      help='Model name')
...
def run(rank, n_gpus, hps):
  ...
  dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:23452', world_size=n_gpus, rank=rank)
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_spk_org_and_H = F.l1_loss(g_H.detach(), g)
        # loss_spk_VQ_and_H = F.l1_loss(g.detach().expand(vq_spk_emb.size()), vq_spk_emb)
        #이거 차원 맞나 확인하기
        
        loss_gen_all = loss_gen + loss_fm + loss_mel + 2*loss_commitment + 2*loss_codebook + loss_spk_org_and_H 
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_commitment, loss_codebook, loss_spk_org_and_H, perplexity]
