...
def main():
  ...
  parser.add_argument('-c', '--config', type=str, default="/home/sim/VoiceConversion/V6/freevc_v6-p.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str, default="V6-p_mel",
                      help='Model name')
...
def run(rank, n_gpus, hps):
  ...
  dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:23458', world_size=n_gpus, rank=rank)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      # Generator -> y_hat
      # g: None, mel: None
      y_hat, ids_slice, (emb, emb_quantized, perplexity), g = net_g(c, spec, g=g, mel=mel)
      ...
      y_hat_mel = mel_spectrogram_torch(
       ...
       #Percussive cycle loss
      y_hat_spec = spectrogram_torch(y_hat.squeeze(1), hps.data.filter_length,
      hps.data.sampling_rate, hps.data.hop_length, hps.data.win_length,
      center=False)
      
      res_hat = hpss_module(y_hat_spec.unsqueeze(1))
      spec_H_hat, spec_P_hat = res_hat['harm_spec'].squeeze(1), res_hat['perc_spec'].squeeze(1)
      
      mel_P_hat = spec_to_mel_torch(
      spec_P_hat, 
      hps.data.filter_length, 
      hps.data.n_mel_channels, 
      hps.data.sampling_rate,
      ...
      mel_P_slice = commons.slice_segments(mel_P, ids_slice, hps.train.segment_size // hps.data.hop_length)
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_commitment, loss_codebook = vq_loss(emb, emb_quantized, commitment_labmda=1, codebook_labmda=0) #VQ_loss
        loss_codebook = torch.tensor([0]).cuda(rank)
        # loss_contrastive = contrastive_loss(vq_spk_emb, spk_emb) #Contrastive loss
        loss_contrastive = torch.tensor([0]).cuda(rank)
        loss_percussive_mel = F.l1_loss(mel_P_slice, mel_P_hat) * hps.train.spec_p
        loss_gen_all = loss_gen + loss_fm + loss_mel + loss_kl + 2*loss_commitment + loss_contrastive + loss_percussive_mel
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_kl, loss_commitment, loss_codebook, loss_contrastive, perplexity, loss_percussive_mel]
        ...
        if hps.setting.log_wandb:
          wandb.log({
            ...
            "loss/percussive": loss_percussive_mel.detach().cpu().numpy(),
            ...
            # "train/gt_mel_H": wandb.Image(mel[0].detach().cpu().numpy()),
...
def evaluate(hps, nets, eval_loader, writer_eval):
    ...
    with torch.no_grad():
      ...
      # spec_P_slice = commons.slice_segments(spec_P, ids_slice, hps.train.segment_size // hps.data.hop_length)
      ...
      # Generator, fmap_r: discriminator의 layer 별 output for y, fmap_g: for y_hat
        ...
        with autocast(enabled=False):
          ...
          loss_percussive = F.l1_loss(spec_P, spec_P_hat) * hps.train.spec_p
    ...
    if hps.setting.log_wandb:
      wandb.log({
            ...
            "eval_loss/g_percussive": losses_list[4].detach().cpu().numpy(),
