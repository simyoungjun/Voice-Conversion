...
def main():
  ...
  parser.add_argument('-c', '--config', type=str, default="/home/sim/VoiceConversion/V6/freevc_v6_ssl.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str, default="V6_ssl",
                      help='Model name')
...
def run(rank, n_gpus, hps):
  ...
  dist.init_process_group(backend='nccl', init_method='tcp://127.0.0.1:23453', world_size=n_gpus, rank=rank)
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_commitment, loss_codebook = vq_loss(emb, emb_quantized) #VQ_loss
        # loss_contrastive = contrastive_loss(vq_spk_emb, spk_emb) #Contrastive loss
        loss_contrastive = torch.tensor([0]).cuda(rank)

        loss_gen_all = loss_gen + loss_fm + loss_mel + loss_kl + 2*loss_commitment + 2*loss_codebook
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_kl, loss_commitment, loss_codebook, perplexity]
