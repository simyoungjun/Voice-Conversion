...
def main():
  ...
  parser.add_argument('-c', '--config', type=str, default="/home/sim/VoiceConversion/V7/freevc_v7.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str, default="V7",
                      help='Model name')
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      # Generator -> y_hat
      # g: None, mel: None
      y_hat, ids_slice, (emb, emb_quantized, perplexity), g = net_g(c, spec, g=g, s=s)
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_gen_all = loss_gen + loss_fm + loss_mel + loss_kl + 2*loss_commitment + loss_contrastive
...
def evaluate(hps, nets, eval_loader, writer_eval):
    ...
    with torch.no_grad():
      ...
      y_hat = generator.module.infer(c, g=g, s=s)
