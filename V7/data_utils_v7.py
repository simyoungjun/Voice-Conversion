...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        if not self.use_sr:
            ...
            s_filename = filename.replace(".wav", ".pt")
            s_filename = s_filename.replace("vctk-16k", "wavlm-6L")
            s = torch.load(s_filename).squeeze(0)
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        s_padded = torch.FloatTensor(len(batch), batch[0][3].size(0), max_spec_len)
        ...
        s_padded = commons.slice_segments(s_padded, ids_slice, spec_seglen)[:,:,:-1]
        
        spec_padded = spec_padded[:,:,:-1]
        wav_padded = wav_padded[:,:,:-self.hps.data.hop_length]

        return c_padded, spec_padded, wav_padded, s_padded
