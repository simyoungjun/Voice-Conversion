...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    if not self.use_spk:
      self.enc_spk = SpeakerEncoder(mel_n_channels=1024,model_hidden_size=gin_channels, model_embedding_size=gin_channels)

  def forward(self, c, spec, g=None, s=None, c_lengths=None, spec_lengths=None):
    '''
    z: z_prime
    ...
    if not self.use_spk:
      g = self.enc_spk(s.transpose(1,2))
    g = g.unsqueeze(-1)
  ...
  def infer(self, c, g=None, s=None, c_lengths=None):
    if c_lengths == None:
      c_lengths = (torch.ones(c.size(0)) * c.size(-1)).to(c.device)
    if not self.use_spk:
      g = self.enc_spk.embed_utterance(s.transpose(1,2))
    g = g.unsqueeze(-1)
