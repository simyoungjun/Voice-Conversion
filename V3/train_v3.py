...
import utils
# 이부분 바뀜
from data_utils_v3 import (
  TextAudioSpeakerLoader,
  TextAudioSpeakerCollate,
  DistributedBucketSampler
)
from V3.models_v3 import (
  SynthesizerTrn,
...
def main():
  ...
  parser.add_argument('-c', '--config', type=str, default="/root/sim/VoiceConversion/FreeVC/configs/freevc_v3.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str, default="v3",
                      help='Model name')
  ...
  hps.train.world_size = n_gpus*hps.train.world_size
  # hps.train.gpu_id = [0, 1, 2]
  # hps.train.num_workers = 4*n_gpus
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    # spec, y = spec.cuda(rank), y.cuda(rank)
    # c = c.cuda(rank)
    # mel, mel_H, spec_P = mel.cuda(rank), mel_H.cuda(rank), spec_P.cuda(rank)
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q) = net_g(spec_P, spec, g=g, mel=mel_H)
      ...
      if global_step % hps.train.log_interval == 0:
        ...
        if hps.setting.log_wandb:
          wandb.log({
            ...
            "train/org_mel": wandb.Image(np.flipud(y_mel[0].detach().cpu().numpy())),
            "train/gen_mel": wandb.Image(np.flipud(y_hat_mel[0].detach().cpu().numpy())),
            "train/gt_mel": wandb.Image(np.flipud(mel[0].detach().cpu().numpy())),
            "train/gt_spec_P": wandb.Image(np.flipud(librosa.amplitude_to_db(spec_P[0].detach().cpu().numpy()))),
            "train/gt_mel_H": wandb.Image(np.flipud(mel_H[0].detach().cpu().numpy())),
...
def evaluate(hps, nets, eval_loader, writer_eval):
    ...
    with torch.no_grad():
      ...
      # c = c[:1].cuda(0)
      # spec, y = spec.cuda(0), y.cuda(0)
      # mel, mel_H, spec_P = mel.cuda(0), mel_H.cuda(0), spec_P.cuda(0)
          # break
        

      # y_hat = generator.module.infer(c, g=g, mel=mel)
      y_hat = generator.module.infer(spec_P, g=g, mel=mel_H)
    ...
    if hps.setting.log_wandb:
      wandb.log({
            ...
            "eval/gen_mel": wandb.Image(np.flipud(y_hat_mel[0].detach().cpu().numpy())),
            "eval/gt_mel": wandb.Image(np.flipud(mel[0].detach().cpu().numpy())),
            "eval/gt_spec_P": wandb.Image(np.flipud(librosa.amplitude_to_db(spec_P[0].detach().cpu().numpy()))),
            "eval/gt_mel_H": wandb.Image(np.flipud(mel_H[0].detach().cpu().numpy())),
            "eval/gt_mel_P": wandb.Image(np.flipud(mel_P[0].detach().cpu().numpy())),
