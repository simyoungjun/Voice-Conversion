...
def evaluate(model, vocoder, eval_data_loader, criterion, global_step, mel_stat, writer=None, DEVICE=None, hps=None):
	...
	with torch.no_grad():
		...
		for step, items in enumerate(eval_data_loader):
			...
			c = c.permute(0,2,1)
			mels_hat, mels_code, mels_style, commitment_loss, perplexity = model.evaluate(c.detach())
		...
		print(f'step: {global_step}, eval_all_loss: {eval_loss / len(eval_data_loader)}, eval_recon_loss: {recon_loss/len(eval_data_loader)}, eval_commit_loss: {commitment_loss/len(eval_data_loader)}')
