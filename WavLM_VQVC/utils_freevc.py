...
def get_content(cmodel, y, layer=None):
    with torch.no_grad():
      if layer == None:
...
def get_vocoder(rank):
    with open("/home/sim/VoiceConversion/FreeVC/hifigan/config.json", "r") as f:
        config = json.load(f)
    config = hifigan.AttrDict(config)
    vocoder = hifigan.Generator(config)
    ckpt = torch.load("/home/sim/VoiceConversion/FreeVC/hifigan/generator_v1")
...
def get_hparams(init=True, args=None):
  ...
  # print(args)
  if args != None:
    model_dir = os.path.join(f"/shared/racoon_fast/sim/Checkpoints/logs", args.model_name)

    if not os.path.exists(model_dir):
      os.makedirs(model_dir)

    config_path = args.config
    config_save_path = os.path.join(model_dir, f"{args.model_name}.json")
