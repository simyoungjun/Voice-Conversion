...
import commons
import V6_3.modules_noIN as modules_noIN
...
'''
  def __init__(self,
    ...
    for i in range(n_flows):
      self.flows.append(modules_noIN.ResidualCouplingLayer(channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=gin_channels, mean_only=True))
      self.flows.append(modules_noIN.Flip())
...
class Encoder(nn.Module):
  def __init__(self,
    ...
    self.enc = modules_noIN.WN(hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=gin_channels)
    self.proj = nn.Conv1d(hidden_channels, out_channels * 2, 1)
    #Vector Quantization
    if vq_codebook_size != None:
        self.codebook = modules_noIN.VQEmbeddingEMA(vq_codebook_size, hidden_channels)
...
class Generator(torch.nn.Module):
    def __init__(self, initial_channel, resblock, resblock_kernel_sizes, resblock_dilation_sizes, upsample_rates, upsample_initial_channel, upsample_kernel_sizes, gin_channels=0):
        ...
        resblock = modules_noIN.ResBlock1 if resblock == '1' else modules_noIN.ResBlock2
    ...
    def forward(self, x, g=None):
        ...
        for i in range(self.num_upsamples):
            x = F.leaky_relu(x, modules_noIN.LRELU_SLOPE)
            x = self.ups[i](x)
            xs = None
...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.enc_p = Encoder(ssl_dim, inter_channels, hidden_channels, 5, 1, 16, vq_codebook_size=None)
    #Decoder
  ...
  def forward(self, c, spec, g=None, mel=None, c_lengths=None, spec_lengths=None):
    ...
    # emb, emb_quantized, perplexity = self.enc_p(c, c_lengths) # _p: prior  distribution
    ...
    return o, ids_slice, (emb, emb_quantized, perplexity), g
  ...
  def infer(self, c, g=None, mel=None, c_lengths=None):
    ...
    emb, emb_quantized, perplexity = self.enc_p(c, c_lengths)
    z = emb_quantized
    o = self.dec(z, g=g)
