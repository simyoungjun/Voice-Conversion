...
class VQEmbeddingEMA(nn.Module):
  ...
  def __init__(self, n_embeddings, embedding_dim, epsilon=1e-5):
    ...
    # self.codebook = torch.nn.Parameter(torch.from_numpy(np.load('/shared/racoon_fast/sim/codebook_init/codebook_1024.npy')), requires_grad=True) # 이부분 code_loss 때문에 코드북 파라미터화 함. 다른 모델들은 그냥 load 해서 하면됨
  ...
  def forward(self, x, metric='L2'):
    ...
    # codebook_change = F.mse_loss(self.codebook.detach(), self.codebook_org.to(x.device).detach())
    # print(f"***Codebook Change: {codebook_change}")
