...
def process(wav_name):
    ...
    # if os.path.exists(wav_path):
        ...
        # wav_trim, _ = librosa.effects.trim(wav, top_db=20)
        peak = np.abs(wav).max()
        if peak > 1.0:
            wav = 0.98 * wav / peak
...
if __name__ == "__main__":
    ...
    parser.add_argument("--in_dir", type=str, default="/shared/racoon_fast/sim/VCTK/wav48_silence_trimmed", help="path to source dir")
    parser.add_argument("--out_dir1", type=str, default="/shared/racoon_fast/sim/VCTK/preprocessed/vctk-16k_no_trim", help="path to target dir")
