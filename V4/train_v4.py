...
import utils
# 이부분 바뀜
from V4.data_utils_v4 import (
  TextAudioSpeakerLoader,
  TextAudioSpeakerCollate,
  DistributedBucketSampler
)
from V4.models_v4 import (
  SynthesizerTrn,
...
def main():
  ...
  parser.add_argument('-c', '--config', type=str, default="/root/sim/VoiceConversion/FreeVC/configs/freevc_v4.json",
                      help='JSON file for configuration')
  parser.add_argument('-m', '--model', type=str, default="v4",
                      help='Model name')
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, items in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q) = net_g(c, spec, g=g, mel=mel_H)
