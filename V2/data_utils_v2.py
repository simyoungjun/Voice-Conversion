...
sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname('/root/sim/VoiceConversion/FreeVC'))))
...
"""
HPSS 적용, Harmonic, Percussive feature를 바로 추출하여 적용
"""
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        audio_norm = audio.astype(np.float32)/self.max_wav_value
        audio_H, audio_P = librosa.effects.hpss(audio_norm)
        
        audio_norm = torch.from_numpy(audio).unsqueeze(0)
        audio_P_norm = torch.from_numpy(audio_P).unsqueeze(0)
        audio_H_norm = torch.from_numpy(audio_H).unsqueeze(0)

        # audio_H_norm = harmonic_audio / self.max_wav_value
        # audio_H_norm = audio_H_norm.unsqueeze(0)
        # audio_P_norm = percussive_audio / self.max_wav_value
        # audio_P_norm = audio_P_norm.unsqueeze(0)
        
        
        mel_P = mel_spectrogram_torch(
            audio_P_norm, 
            self.filter_length,
            self.n_mel_channels,
            self.sampling_rate,
        ...
        mel_P = mel_P.squeeze(0)
        
        mel_H = mel_spectrogram_torch(
            audio_H_norm, 
            self.filter_length,
            self.n_mel_channels,
            self.sampling_rate,
        ...
        mel_H = mel_H.squeeze(0)
        
        # torch.FloatTensor(data.astype(np.float32)), sampling_rate
        ...
        #     spk_filename = spk_filename.replace("dataset/vctk-16k", "dataset/spk")
        #     spk = torch.from_numpy(np.load(spk_filename))
        
        # if not self.use_sr:
        #     c_filename = filename.replace(".wav", ".pt")
        #     c_filename = c_filename.replace("dataset/vctk-16k", "dataset/wavlm")
        ...
        #     c_filename = c_filename.replace("dataset/vctk-16k", "dataset/sr/wavlm")
        #     c = torch.load(c_filename).squeeze(0)
        ...
        return mel_P, spec, audio_norm, mel_H
...
class TextAudioSpeakerCollate():
    ...
    def __call__(self, batch):
        ...
        batch: [P_normalized, spec_normalized, wav_normalized, H_normalized]
        ...
        H_padded = torch.FloatTensor(len(batch), batch[0][0].size(0), max_spec_len)
        ...
        H_padded = commons.slice_segments(H_padded, ids_slice, spec_seglen)[:,:,:-1]
        
        if self.use_spk:
          return c_padded, spec_padded, wav_padded, spks
        else:
          return c_padded, spec_padded, wav_padded, H_padded
